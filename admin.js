...
function checkAuth() {
  if (sessionStorage.getItem('clb_auth') === '1') {
...
async function login() {
  ...
  try {
    ...
    if (j.ok) { sessionStorage.setItem('clb_auth', '1'); checkAuth(); }
    else { loginError.style.display = 'block'; }
  } catch (e) {
    // Network fallback so a CF outage doesn\'t lock the owner out.
    if (pw === ADMIN_PASSWORD) { sessionStorage.setItem('clb_auth', '1'); checkAuth(); }
    else { loginError.style.display = 'block'; }
  }
}
document.getElementById('logoutBtn').addEventListener('click', () => {
  sessionStorage.removeItem('clb_auth');
  location.reload();
});
...
function generateDescription(name, cat) {
  ...
  const colors = { black: 'sleek black', white: 'crisp white', navy: 'deep navy', grey: 'soft grey', gray: 'soft grey', blue: 'rich blue', pink: 'pretty pink', blush: 'soft blush', red: 'bold red', green: 'fresh green', sage: 'sage green', emerald: 'emerald', burgundy: 'rich burgundy', wine: 'wine red', nude: 'soft nude', beige: 'warm beige', cream: 'soft cream', lilac: 'soft lilac', purple: 'rich purple', yellow: 'sunny yellow', orange: 'warm orange', floral: 'floral', brown: 'warm brown', gold: 'golden', champagne: 'champagne' };
  let color = '';
  for (const c in colors) if (lower.includes(c)) { color = colors[c]; break; }

  const catMap = {
    Dresses: 'dress', Tops: 'top', 'Two Piece Sets': 'two piece set', Jumpsuits: 'jumpsuit',
    Skirts: 'skirt', Trousers: 'pair of trousers', Jeans: 'pair of jeans', Shorts: 'pair of shorts',
    Outerwear: 'piece', Knitwear: 'knit', Activewear: 'set', Accessories: 'piece',
  };
  const type = catMap[cat] || 'piece';

  const openers = [
    `Beautiful ${color || 'brand new'} ${type}, hand-picked to make you feel your best.`,
    `A ${color || 'gorgeous'} ${type} that turns heads. Brand new, ready to wear.`,
    `${color ? color.charAt(0).toUpperCase() + color.slice(1) : 'Fresh in'} ${type}, styled to flatter and made to be loved.`,
  ];
  const mids = [
    `Brand new and quality-checked before listing.`,
  ...
  const closes = [
    ...
    `Visit us at Mithoo Biashara Center, 2nd floor, Shop S-05, or we deliver.`,
  ];
...
function payLabel(m) { return m === 'mpesa' ? 'M-Pesa' : 'Cash'; }
document.getElementById('salePay')?.addEventListener('click', e => {
  const b = e.target.closest('.pay-btn'); if (!b) return;
  salePayMethod = b.dataset.pay; setPayToggle('salePay', salePayMethod);
});
...
async function recordSale(withBuyer) {
  ...
  try {
    ...
    lastSale = { name: soldBag ? soldBag.name : '', size, qty, amount: salePrice, paid: amountPaid, balance, paymentMethod: sale.paymentMethod, buyerName: sale.buyerName, buyerPhone: sale.buyerPhone, soldAt: sale.soldAt };
...
function receiptText(s) {
  ...
  const lines = [
    ...
    `Total: ${fmtKsh(total)}. Paid by ${payLabel(s.paymentMethod)}.`,
  ];
  if (s.balance > 0) lines.push(`Paid now: ${fmtKsh(s.paid)}. Balance owing: ${fmtKsh(s.balance)}.`);
  lines.push(`Thank you for shopping with us. ${CLB_SHOP.addr1}, ${CLB_SHOP.addr2}.`);
  return lines.join('\n');
}
...
function showReceipt(s) {
  ...
  document.getElementById('receiptSummary').innerHTML =
    `<strong>${escapeHtml(s.name)}</strong> · Size ${escapeHtml(s.size)} · ${s.qty} item(s)<br>${fmtKsh(total)}${s.balance > 0 ? `<br><span class="owed-amount">Paid ${fmtKsh(s.paid)} · still owes ${fmtKsh(s.balance)}</span>` : ''}`;
...
function printReceipt() {
  ...
  document.getElementById('receiptPrint').innerHTML = `
    <div class="rcpt">
      ...
      <div class="rcpt-row"><span>Paid by</span><span>${payLabel(s.paymentMethod)}</span></div>
...
function openEditSale(bagId, soldAt) {
  ...
  editSalePayMethod = s.paymentMethod || 'cash';
  setPayToggle('editSalePay', editSalePayMethod);
...
document.getElementById('editSaleSaveBtn').addEventListener('click', async () => {
  ...
  try {
    await apiMutateAndPublish(() => {
      ...
      s.paymentMethod = editSalePayMethod;
...
function renderDashboard() {
  ...
  // sold items that have a buying price recorded. costKnown = how many of the
  // sold items carry a cost; soldItemsCount = all items with at least one sale
  // (for the coverage note, so a partial figure isn't mistaken for total profit).
  ...
  // Today's takings split by payment method (sales with no method recorded fall into Cash).
  const tStart = startOfDay(now);
  let cashT = 0, mpesaT = 0, nToday = 0, owedToday = 0;
  bags.forEach(bag => (bag.sales || []).forEach(s => {
    if (new Date(s.soldAt) >= tStart) {
      ...
      nToday += Number(s.qty) || 1;
    }
    (s.payments || []).forEach(p => {
      if (new Date(p.at) >= tStart) {
        const amt = Number(p.amount) || 0;
        if (p.method === 'mpesa') mpesaT += amt; else cashT += amt;
      }
    });
  }));
  const splitEl = document.getElementById('takingsSplit');
  if (splitEl) splitEl.innerHTML = nToday
    ? `<span class="takings-label">Today's takings</span><span class="takings-chip cash">💵 Cash ${fmtKsh(cashT)}</span><span class="takings-chip mpesa">📱 M-Pesa ${fmtKsh(mpesaT)}</span><span class="takings-chip total">${nToday} sold</span>${owedToday > 0 ? `<span class="takings-chip owed">📝 On credit ${fmtKsh(owedToday)}</span>` : ''}`
    : '';
...
window.bulkSell = () => {
  ...
  bulkSellPayMethod = 'mpesa';
  setPayToggle('bulkSellPay', 'mpesa');
...
async function commitBulkSold(withBuyer) {
  ...
  const payMethod = bulkSellPayMethod;
...
document.getElementById('bulkSellPay')?.addEventListener('click', e => {
  const b = e.target.closest('.pay-btn'); if (!b) return;
  bulkSellPayMethod = b.dataset.pay; setPayToggle('bulkSellPay', bulkSellPayMethod);
});
...
document.getElementById('payDebtPay')?.addEventListener('click', e => { const b = e.target.closest('.pay-btn'); if (!b) return; payDebtMethod = b.dataset.pay; document.querySelectorAll('#payDebtPay .pay-btn').forEach(x => x.classList.toggle('active', x === b)); });
...
window.remindDebt = phone => {
  ...
  const intro = n === 1
    ? `A friendly reminder about your balance on the item you took from Chic La Belle:`
    : `A friendly reminder about the ${n} items you took from Chic La Belle that still have a balance:`;
...
const INSIGHTS_KEY = 'clb_analytics';
...
const IG_USER_ID = '68768998504';
const APPAREL_CATEGORIES = ['Dresses', 'Tops', 'Two Piece Sets', 'Jumpsuits', 'Skirts', 'Trousers', 'Jeans', 'Shorts', 'Outerwear', 'Knitwear', 'Activewear', 'Accessories'];
...
function renderIgSyncList() {
  igSyncListEl.innerHTML = igSyncCandidates.map((it, i) => {
    ...
    // Chic La Belle is NEW-STOCK: stock is { "M":1, "L":1, "XL":1 } etc. Show every
    ...
    const catOpts = APPAREL_CATEGORIES.map(c => `<option value="${c}" ${c === s.category ? 'selected' : ''}>${c}</option>`).join('');
    return `
