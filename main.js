...
(async function () {
  ...
  const NEW_DAYS = 7;
  const LOW_STOCK = 3;
  const WISHLIST_KEY = 'clb_wishlist';
  let items = [];
  let settings = {};
  let suspended = false;
  ...
  const ANALYTICS_KEY = 'clb_analytics';
  ...
  function enquireBody(item, soldOut, selectedSize) {
    ...
    return soldOut
      ? `Hi Chic La Belle! I saw *${item.name}* is sold out. Will it be back in stock? I'd love to reserve one.`
      : `Hi Chic La Belle! I'd like to enquire about *${item.name}*${sizePart}${pricePart} from your catalog.`;
  }

  function whatsappLink(item, soldOut, selectedSize) {
    const phone = settings.whatsappNumber || '254112199913';
  ...
  document.getElementById('wishlistEnquireAll')?.addEventListener('click', e => {
    ...
    const msg = `Hi Chic La Belle! I'd like to enquire about these saved items:\n\n${lines.join('\n')}\n\nAre they available?`;
  ...
  function showSuspended() {
    ...
    const shopName = settings.shopName || settings.businessName || 'Chic La Belle';
    const tagline = settings.tagline || "Women's Outfits";
    const logoUrl = 'images/logo-nav.jpg';
    document.title = shopName + ' · Paused';

    // Prospect-demo paused page: pitch the one-off option + WhatsApp us to switch it back on.
