...
## Pending / follow-ups
- **GitHub repo + Actions auto-deploy:** `.github/workflows/deploy.yml` is committed but inert until the repo is pushed to GitHub and the reusable `CLOUDFLARE_API_TOKEN` secret is added (gh CLI is not installed on this machine). Until then, deploy manually: worker `cd worker && npx wrangler deploy`; site `CLOUDFLARE_ACCOUNT_ID=58685495706b973821d77208248c66fc npx wrangler pages deploy . --project-name=chic-la-belle --branch=main --commit-dirty=true`.
- **Custom domain** (`chiclabelle.essenceautomations.com` or similar): add via CF Pages dashboard so the proxied CNAME auto-creates (wrangler/API can't create the DNS record — CATALOG-STANDARDS hosting). Then update the hardcoded `https://chic-la-belle.pages.dev` URLs (worker SITE, index OG, admin broadcast lookUrl, main.js).
